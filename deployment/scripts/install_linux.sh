...
if [ ! -f "$PROJECT_ROOT/config/config.ini" ]; then
  echo "[WARN] config/config.ini not found. Creating a default template."
  mkdir -p "$PROJECT_ROOT/conf"
  cat > "$PROJECT_ROOT/config/config.ini" << EOF
[Paths]
library_dir = /path/to/your/library
output_dir = /path/to/output
EOF
  echo "[INFO] A default config.ini has been created at config/config.ini. Please edit it with your custom paths."
else
  echo "[INFO] Please edit 'config/config.ini' with your custom paths before running."
fi
