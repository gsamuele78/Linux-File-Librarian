...
REPORT_DIR="$PROJECT_ROOT/monitoring/logs"
...
EOF
    ...
    if [ -f "$LOG_DIR/performance_metrics.json" ]; then
        python3 "$PROJECT_ROOT/src/services/generate_report.py" "$LOG_DIR/performance_metrics.json" "$REPORT_DIR/performance_report.txt"
    else
...
main() {
    ...
    if [ -f "deployment/scripts/cleanup_logs.sh" ]; then
        bash deployment/scripts/cleanup_logs.sh
    fi
    ...
    if python3 -m src.enterprise.professional_orchestrator; then
...
CONFIGURATION:
    Edit config/config.ini to configure source paths and library destination.

REPORTS:
    - Processing report: librarian_professional_report.json
    - Performance report: monitoring/logs/performance_report.txt
    - Logs: monitoring/logs/professional_run.log
